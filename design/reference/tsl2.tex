...
declaration in the current and parent templates using 
Algorithm~\ref{a:method}.

\begin{algorithm}
    \begin{algorithmic}[1]
        \Statex {\bf Input:} Child method body $M$ and parent method 
        body $M'$, where $M$ is either a complete body $(b)$ or partial
        body $(p,e)$.  Likewise, $M'$ is either $(b')$ or $(p',e')$ or
        $nil$, if $M$ is not an overloaded method.
        \Statex {\bf Output:} Full child method body
        \Function{fullBody}{$M, M'$}
          \Comment{Recursively invoke the algorithm on $M'$}
          \Switch{(M, \Call{fullBody}{$M'$})}
            \Case{$(nil, M')$}
              \State {\bf return} $nil$
            \EndCase
            \Case{$(M, nil)$}
              \Comment{The method is not an overloaded method}
              \State {\bf return} $M$
            \EndCase
            \Case{$(M, (b'))$} 
              \Comment{Parent method has a complete body: override it}
              \State {\bf return} $M$
            \EndCase
            \Case{$((p,e), (p',e'))$} 
              \Comment{Both parent and child have partial bodies: merge 
                       preambles and epilogues using sequential composition}
              \State {\bf return} $((p';p), (e;e'))$
            \EndCase
            \Case{$((b), (p',e'))$} 
              \Comment{Child has a complete body, parent has partial body: 
                       prepend and append parent's preamble and epilogue to 
                       the child}
              \State {\bf return} $(p';b;e')$
            \EndCase
            \Case{\bf otherwise}
              \State {\bf error}
            \EndCase
